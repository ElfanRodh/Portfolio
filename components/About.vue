    ...
    <div class="max-w-screen-2xl mx-auto pt-28 pb-10 px-4 sm:pt-24 sm:px-6 md:pt-16 md:pb-16 lg:pt-28 lg:px-8 xl:pt-28">
      <div class="sm:text-center lg:text-left z-0">
        <span class="before:block before:absolute before:left-0 before:-bottom-3 before:h-2 before:w-10 before:bg-gray-500 dark:before:bg-indigo-100 before:rounded-full relative inline-block">
          <span class="relative text-4xl font-bold text-sky-600 my-3 font-title">About Me</span>
        </span>
      </div>
      <figure class="md:flex rounded-xl p-8 md:p-0">
        ...
        <div class="pt-6 md:p-8 text-center md:text-left space-y-4">
          <div class="text-left mb-10">
            <h3 class="text-center md:text-left text-2xl font-semibold text-teal-500 my-3 font-mono">
              <span>{{ about.name }}</span>
            </h3>
            <p class="text-center md:text-left text-gray-500 dark:text-blue-50 leading-12 text-2xl">
              <span>
                {{ about.bio }}
              </span>
            </p>
            <div class="w-full md:w-1/2 h-1 mt-2 rounded-full bg-teal-500"></div>
          </div>

          <div class="text-center md:text-left mb-10">
            <span class="font-bold text-blue-500">Region: </span>
            <span class="font-medium text-2xl text-gray-500 dark:text-blue-50 block mb-2">{{ about.region }}</span>

            <span class="font-bold text-blue-500">City: </span>
            <span class="font-medium text-2xl text-gray-500 dark:text-blue-50 block mb-2">{{ about.city }}</span>

            <span class="font-bold text-blue-500">Age: </span>
            <span class="font-medium text-2xl text-gray-500 dark:text-blue-50 block mb-2">{{ age }}</span>

            <span class="font-bold text-blue-500">Address: </span>
            <span class="font-medium text-2xl text-gray-500 dark:text-blue-50 block mb-2">
              {{ about.address }}
            </span>
          </div>
      ...
      <div class="md:flex">
        ...
        <div class="w-full md:w-4/12 px-3">
          <div class="pt-6 text-center md:text-left space-y-4">
            <div class="text-left mb-10">
              <h3 class="text-2xl font-semibold text-teal-500 my-3 font-mono inline-flex">
                <span>
              ...
              <div v-for="(education, index) in educations" :key="index" class="py-2 min-h-6">
                <p class="text-center md:text-left text-gray-500 dark:text-blue-50 leading-12 text-2xl">
                  <span> {{ education.yearEnd }} - {{ education.name }} ({{ education.subtext }}) </span>
                </p>
                <div class="w-full md:w-1/2 h-1 mt-2 rounded-full bg-teal-500"></div>
              </div>
              <!-- <div class="py-2 min-h-6">
                <p
                  class="text-center md:text-left text-gray-500 dark:text-blue-50 leading-12 text-2xl"
                >
                  <span>
        ...
        <div class="w-full md:w-4/12 px-3">
          <div class="pt-6 text-center md:text-left space-y-4">
            <div class="text-left mb-10">
              <h3 class="text-2xl font-semibold text-indigo-500 my-3 font-mono inline-flex">
                <span>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div v-for="(publication, index) in publications" :key="index" class="py-2">
                <p class="text-center md:text-left text-gray-500 dark:text-blue-50 leading-12 text-2xl flex">
                  <span class="w-5/5 md:w-4/5"> {{ publication.yearEnd }} - {{ publication.name }} </span>
        ...
        <div class="w-full md:w-4/12 px-3">
          <div class="pt-6 text-center md:text-left space-y-4">
            <div class="text-left mb-10">
              <h3 class="text-2xl font-semibold text-blue-500 my-3 font-mono inline-flex">
                <span>
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path
              ...
              <div v-for="(certificate, index) in certificates" :key="index" class="py-2">
                <p class="text-gray-500 dark:text-blue-50 leading-12 text-2xl flex">
                  <span class="w-5/5 md:w-4/5"> {{ certificate.yearEnd }} - {{ certificate.name }} </span>
                </p>
...
export default {
  ...
  async fetch() {
    const [about, achievements] = await Promise.all([axios.get(`${process.env.apiUrl}about`), axios.get(`${process.env.apiUrl}achievements?sortby=yearEnd&order=desc`)]);
